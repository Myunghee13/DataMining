...
case_number = int(sys.argv[1])  # 1 or 2
...
def PCY(iterator):
    ...
    dic = Counter()
    pcy_counter = Counter()  ## PCY: counting bucket
    for basket in partition:
        dic.update(basket)

        for pair in itertools.combinations(basket, 2):  ##
            hash_value = str((hash(pair[0]) + hash(pair[1])) % 250)  ## hash function
            pcy_counter[hash_value] += 1  ##

    bitmap0_list = []  ##
    for key in pcy_counter:  ##
        if pcy_counter[key] < p:  ##
    ...
    for key in itertools.combinations( \
        ...
        if ((hash(key[0]) + hash(key[1])) % 250) not in bitmap0_list:  ##
            key = tuple(sorted(key))
            for basket in partition:
                if set(key).issubset(basket):
                    if key in dic:
...
if case_number == 1:
    baskets = rdd.groupByKey().map(lambda x: (x[0], set(x[1]))).map(lambda x: x[1]).cache()
    ...
    baskets = rdd.map(lambda x: [x[1], x[0]]).groupByKey().map(lambda x: (x[0], set(x[1]))).map(lambda x: x[1]).cache()
    n_partitions = baskets.getNumPartitions()
    p = support / n_partitions
